  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        tableBody.innerHTML =
          '<tr><td colspan="5" class="error">Error fetching top cryptocurrencies. Please try again later.</td></tr>';
        console.error("Request failed with status:", xhr.status);
      }
    }
  };
...
function renderTable() {
  ...
  cryptoData.forEach((crypto) => {
    ...
    const marketCap = parseFloat(crypto.market_cap).toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    });
    const priceChange = parseFloat(crypto.price_change_percentage_24h).toFixed(
      2
    );
    const priceChangeClass = priceChange >= 0 ? "positive" : "negative";
