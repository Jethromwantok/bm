class Account {
  ...
  updateProfile(newName, newEmail, newPhoneNumber) {
    ...
    console.log(
      `Profile updated: ${this.name}, ${this.email}, ${this.phoneNumber}`
    );
  }
  viewProfile() {
...
class Staff extends Account {
  ...
  viewRegistrationRequests() {
    ...
    this.registrationRequests.forEach((req, idx) => {
      console.log(
        `${idx + 1}. ${req.student.name} requests to enroll in ${req.course}`
      );
    });
  }
  ...
  rejectRegistration(index) {
    ...
    if (req) {
      console.log(
        `Rejected: ${req.student.name} not enrolled in ${req.course}`
      );
      this.registrationRequests.splice(index, 1);
    }
  }
  ...
  receiveRegistrationRequest(student, course) {
    ...
    console.log(
      `Received registration request from ${student.name} for ${course}`
    );
  }
